...
if (isServer) then
{
	GRAD_fnc_callFireworksServer = compile preprocessFileLineNumbers "GRAD_fireworks\fireworksServer.sqf";
};
...
// addaction example for remote control infostand - executes GRAD_fnc_callFireworksServer on server - server then calls GRAD_fnc_callFireworksClient on all clients
firebase addAction [ "<t color=""#93E352"">Shoot random with random color ", { [ getPos ( selectRandom [boat1,boat2,boat3] ), 'random', 'random'] remoteExec ['GRAD_fnc_callFireworksServer', 2 ]; }, _Args, 1, false, false, "" ];
